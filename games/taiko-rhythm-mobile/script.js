...
window.addEventListener('keydown', (e) => {
    ...
    // スマホ版でも一応デバッグ用にキーボード入力は残しておく（任意で削除可）
    if (keyMap[e.key]) {
        handleInput(e.key);
    }
});

// スマホ版特有のタッチ（クリック）イベントの追加
function setupTouchControls() {
    const drumLeftKa = document.getElementById('drum-left-ka');
    const drumLeftDon = document.getElementById('drum-left-don');
    const drumRightDon = document.getElementById('drum-right-don');
    const drumRightKa = document.getElementById('drum-right-ka');

    const handleTouch = (key) => (e) => {
        e.preventDefault(); // デフォルトのスクロールやズームなどを防ぐ

        // 最初のタッチ時にAudioContextを再開するための処理
        if (!isGameActive && !startScreen.classList.contains('hidden')) {
            // 空の処理でとりあえず音を鳴らす等の初期化ができるかもしれない
            return;
        }
    ...
    // pointerdown を使うことで、マウスとタッチの療法に対応可能
    drumLeftKa.addEventListener('pointerdown', handleTouch('d'));
    drumLeftDon.addEventListener('pointerdown', handleTouch('f'));
    drumRightDon.addEventListener('pointerdown', handleTouch('j'));
    drumRightKa.addEventListener('pointerdown', handleTouch('k'));
}
